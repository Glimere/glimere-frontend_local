...
const ShippingEditModal: NextPage<Props> = ({ children, address }) => {
  const [toggleEdit, setToggleEdit] = useState(false);
  const [isEditMode, setIsEditMode] = useState(false);
  const [currentEditId, setCurrentEditId] = useState<string | null>(null);

  const jwt = useJwt();
  const { addresses ,updateAddress } = useShippingAddressStore();
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    if (isEditMode && currentEditId) {
      if (jwt) {
        updateAddress({ ...values, _id: currentEditId }, jwt);
        setToggleEdit(false);
        setShippingAddress({...values, _id: currentEditId})
      } else {
  ...
  const handleEditClick = (address: any) => {
    ...
    setCurrentEditId(address._id); // Store the _id of the selected address
    setIsEditMode(true);
  };
  ...
  return (
    <>
      <CustomDialog
        trigger={<div onClick={() => handleEditClick(address)}>{children}</div>}
        triggerState={toggleEdit}
        setTriggerState={setToggleEdit}
      >
        <DialogHeader>
          <DialogTitle className="text-primary-100">Edit Address</DialogTitle>
          <DialogDescription>
            {isEditMode
          ...
          >
            ...
            <DialogFooter>
              <Button type="submit" className="bg-primary-100">
                {isEditMode ? "Save Changes" : "Save New Address"}
              </Button>
              <DialogClose asChild>
