...
import { auth, db, googleProvider } from "@/config/firebase"; // Ensure correct paths
...
export default function Login() {
  ...
  const { setUser, setAuthToken, fetchUser } = useUserStore();
  ...
  const handleUserChange = ({
    ...
    const { name, value } = target;
    setRegisterUserDetails((currentUser) => ({
      ...currentUser,
      [name]: value,
    }));
  };
  ...
  return (
    <div className="w-full h-[100vh] z-[3]">
      <div
        className={`absolute h-full p-6 sm:flex hidden bg-cover bg-center bg-no-repeat z-10 w-[50%] duration-500 ease-in-out card ${
      ...
      <div className="w-full h-full flex flex-row lg:px-[6.8rem]">
        {/* Signup Section */}
        <div
          className={`sm:flex-[1] sm:w-[50%] sm:p-[40px] ${
        ...
        <div
          className={`sm:flex-[1] sm:w-[50%] sm:p-[40px] ${
            isVisible ? "w-[0%] p-0 flex-[0] overflow-hidden" : "w-full"
          } flex flex-col justify-center items-center`}
        >
          <div
